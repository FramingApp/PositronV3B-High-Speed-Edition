...
m3_dia = 2.73;		// 3mm hole diameter
m3_nut_hex = 1;		// 1 for hex, 0 for square nut
m3_nut_flats = 5.7;	// normal M3 hex nut exact width = 5.5
m3_nut_depth = 2.7;	// normal M3 hex nut exact depth = 2.4, nyloc = 4

retainer = 1;		// Belt retainer above teeth, 0 = No, 1 = Yes
retainer_ht = 0.6;	// height of retainer flange over pulley, standard = 1.5
idler = 1;			// Belt retainer below teeth, 0 = No, 1 = Yes
idler_ht = 0.6;		// height of idler flange over pulley, standard = 1.5

pulley_t_ht = 3.2;	// length of toothed part of pulley, standard = 12
pulley_b_ht = 3.3+idler_ht;		// pulley base height, standard = 8. Set to same as idler_ht if you want an idler but no pulley.
pulley_b_dia = 8.6;	// pulley base diameter, standard = 20
...
nut_elevation = pulley_b_ht+(pulley_t_ht/2);//pulley_b_ht/2;
...
GT2_2mm_pulley_dia = tooth_spacing (2,0.254)*0.98;//FUDGE FACTOR
	 ...
	 {	 
		union()
		{
			...
			if ( pulley_b_ht < 2 ) { echo ("CAN'T DRAW PULLEY BASE, HEIGHT LESS THAN 2!!!"); } else {
				rotate_extrude($fn=pulley_b_dia*20)
				{
						square([pulley_b_dia/2-1,pulley_b_ht]);
						square([pulley_b_dia/2,pulley_b_ht]);
						translate([pulley_b_dia/2,pulley_b_ht-1]) circle(0);
				}
			}
		...
		translate([0,0,-1])cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,$fn=motor_shaft*20);
		...
		//if ( (pulley_b_dia - motor_shaft)/2 < m3_nut_depth + 3 ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
			...
			union()
			{
				...
				//grub screw hole
				rotate([0,0,22.5])cylinder(r=m3_dia/2,h=50,$fn=40);
			}
		}}
